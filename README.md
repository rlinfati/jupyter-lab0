...
## Jupyter Volume Shared
- sudo podman volume create --ignore jupyter-shared
- sudo mkdir -p /var/lib/containers/storage/volumes/jupyter-shared/_data/.ipynb_checkpoints
- sudo chown root  /var/lib/containers/storage/volumes/jupyter-shared/_data
- sudo chgrp users /var/lib/containers/storage/volumes/jupyter-shared/_data
- sudo chmod 1775  /var/lib/containers/storage/volumes/jupyter-shared/_data
...
- sudo chgrp users /var/lib/containers/storage/volumes/$i/_data
...
## Jupyter Secrets users
- printf superadmin@domain.tld | sudo podman secret create J0-admin_users -
## Jupyter Secrets docker
- printf '%s\n' \
- quay.io/jupyter/base-notebook:ubuntu-22.04 \
- quay.io/jupyter/minimal-notebook:ubuntu-22.04 \
- quay.io/jupyter/julia-notebook:ubuntu-22.04 \
- quay.io/jupyter/scipy-notebook:ubuntu-22.04 \
- quay.io/jupyter/r-notebook:ubuntu-22.04 \
...
- sudo podman push docker.io/rlinfati/jupyter-lab0:lab0lab-2024-xx docker.io/rlinfati/jupyter-lab0:lab0lab-2024-xx
...
- sudo podman build --tag jupyter-lab0:lab0-2024-xx    --secret id=GUROBIWLS \
                                                     github.com/rlinfati/jupyter-lab0 --file Dockerfile.LAB0
...
## Jupyter CUDAlab Run
- sudo podman create --interactive --tty --oom-score-adj 1000 --init --name jupyter-CUDAlab --replace --publish 8888:8888 --volume jupyter-user-CUDAlab:/mnt --volume jupyter-shared:/media --device=nvidia.com/gpu=all docker.io/rlinfati/jupyter-lab0:cudalab-2024-xx
